...
with open("book.txt") as book:
    ...
    #using the lambda function in order to define inline
    #a function to search inside the paragraphs and splitting the spaces in between
    words = list(map(lambda p: list(map(lambda s: s.split(" "), p)), paragraphs)) #split the paragraphs into sentences
    flag = " ".join(words[key[0] - 1][key[1] - 1][key[2] - 1] for key in keys) #get the words from the keys array [i-1] to avoid going out of bounds
print(flag)